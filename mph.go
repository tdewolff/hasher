// COPIED FROM https://github.com/dgryski/go-mph 19-DEC-2019
// MIT LICENCED BY DGRYSKI
...
func (t *Table) Query(k string) int32 {
	...
	i = _xorshiftMult64(uint64(seed)+hash) & (size - 1)
	return t.values[i]
}

func _xorshiftMult64(x uint64) uint64 {
	x ^= x >> 12 // a
	x ^= x << 25 // b
	x ^= x >> 27 // c
